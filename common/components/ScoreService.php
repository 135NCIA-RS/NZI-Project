...
class ScoreService
{
	...
	public static function getElementsByScoreType(ScoreTypeEnum $score_type, $sort = true)
	{
		if ($sort)
		{
			$type = (int)$score_type->getValue();
			$sql = "SELECT COUNT(*) AS `count`, `element_id`, `element_type` FROM `scores` WHERE `score_type`={$type} GROUP BY `element_id` ORDER BY 1 DESC";
		...
		foreach ($ptr as $row)
		{
			...
			$enumElemType = ScoreElemEnum::search($row->element_type);
			$score->element_type = ScoreElemEnum::$enumElemType();
			$ptr2[] = $score;
		}
		return $ptr2;
	}
